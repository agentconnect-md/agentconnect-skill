...
| `listAgentHooks`            | `agentId`                                                                                                    | An agent's triggers — webhooks and code-host (GitHub/GitLab) subscriptions.                                                                                                                                                                             |
| `listHookRuns`              | `hookId`                                                                                                     | One trigger's delivery/run history, newest first.                                                                                                                                                                                                       |
| `listGithubInstallations`   | —                                                                                                            | The org's live installations of the deployment GitHub App (owner, repo selection, PR/Checks permission). 404 ⇒ the deployment has no GitHub App.                                                                                                        |
| `getGithubApp`              | —                                                                                                            | Whether the deployment has a GitHub App, its slug, and a fresh one-shot org-bound `installUrl` (~15 min) to install it on GitHub — hand it to the user verbatim when no installation covers a repo's owner. 404 ⇒ no App; 403 ⇒ viewers cannot mint it. |
| `getOperation`              | `operationId`                                                                                                | The state of one of YOUR side-effecting operations — `awaiting_confirmation` / `executing` / `completed` / `failed` / `denied` / `ambiguous` — plus its bounded result. Scoped to this conversation.                                                    |
| `listOperations`            | —                                                                                                            | Your operations in this conversation still waiting for the owner's decision.                                                                                                                                                                            |
| `getGithubRepositoryAccess` | `installationId`, `owner`, `repo`                                                                            | YOUR OWN effective GitHub permission on one repository (admin/write/read/none, canRead, canWrite). A workspace at `write` is granted under the caller's identity, so check this BEFORE asking for one; 404 ⇒ the deployment does not gate per user.     |
| `listGithubRepositories`    | `installationId` (the row `id` above), `page?`, `perPage?` (≤100)                                            | Repositories one installation grants, each with its default branch.                                                                                                                                                                                     |

### Write tools

May be hidden entirely when the credential is read-only (`mcp:read` without
`mcp:write`). Credential, member, organization, access-control, and bot **writes are deliberately
not exposed**, and neither is creating a generic **webhook** trigger (it would mint an
ingress URL and a signing secret) — direct users to the console for those. Code-host
triggers and workspaces DO have tools: see `createGithubTrigger` and
`setAgentWorkspace` below.

| Tool                  | Arguments                                                                                                                                                                                                                                                                                                                                                                                           | Notes                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createAgent`         | `name` (immutable slug), `runtime` (from the daemon's reported runtimes), `displayName?`, `description?`, `model?`, `reasoningEffort?`, `outputMode?` (none/minimal/low/medium/high), `fastMode?`, `permissionMode?`, `placementKind?` (`daemon`/`pool`/`set`), `daemonId?` (omit → unplaced), `setId?`, `pause?`, `workspace?` (see `setAgentWorkspace`)                                           | Env vars, secrets, MCP servers, skills, memory and sharing are console-only. Give an agent that works on one repository its `workspace` HERE — one call instead of a follow-up.                                                                                                                                                                                                                                             |
| `setAgentWorkspace`   | `agentId`, `mode` (`git`/`scratch`), and for `git`: `gitRepo` (cloneable URL, or `owner/repo` for github.com), `gitBranch?` (omit → repo default), `agentDir?`, `worktree?` (per-session checkout), `access?` (`read`/`write`; omit → the highest the target can carry)                                                                                                                             | Replaces the workspace. Provenance is derived from the address — a github.com repo inside an installation or a managed GitLab project gets managed credentials, anything else is an anonymous read-only clone. The CALLER must hold the requested permission. Changing repo or branch discards the daemon-local checkout.                                                                                                   |
| `createGithubTrigger` | `agentId`, `name`, `repoFullName` (`owner/repo`), `family` (`pull_request`/`issues`/`push`/`deployment`), `events` (`family:action` or `family:*`), `commentFamilies?`, `labelFilter?`, `mentionOnly?`, `reviewPolicy?` (`off`/`comment`/`request_changes`/`full`), `reportingMode?` (`off`/`check`/`status`), `gateMode?`, `enabled?`, `targetPlatform?`, `targetChannel?`, `targetIntegrationId?` | One trigger covers ONE family (a second on the same family is a 409). Watching a family from its opening is `["<family>:opened"]`; on every update it is `["<family>:*", "issue_comment:created"]` + `commentFamilies: ["<family>"]`. Reviews and run reporting are pull-request-only and need the workspace repo at `write`. The repo must be covered by an installation (`listGithubInstallations`) or the call is a 400. |
...
### Owner approval (webchat conversations)

In a private webchat conversation your credential is a DELEGATED grant, and a write
does not execute in your own request: it answers `{status:'awaiting_confirmation',
operationId}` and the conversation's owner approves it on a card above the composer.

1. Tell the user in one line what is waiting; never re-issue the write to check on
   it — a fresh call enqueues a SECOND operation.
2. The decision comes back as an `[approval]` line in the conversation. Read the
   outcome with `getOperation` and continue where you stopped.
3. `denied` means the user declined: stop and ask, do not retry. `ambiguous` means
   the outcome is genuinely unknown: read state before touching anything.
